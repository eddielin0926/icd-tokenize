...
icd_df = pd.DataFrame(icd_df['中文診斷'].str.normalize(
    'NFKC').to_list(), columns=['diagnosis'])
...
console.print(
    f'Write into file:\t [cyan bold]{os.path.join(dir_path, "icd.csv")}[/]')
console.print(
    f'There are {len(non_icd_list)} words not in the icd dictionary.')
console.print(
